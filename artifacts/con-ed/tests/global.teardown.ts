 ...
 *   repayment_guarantees → receipts → reimbursements → request events → allocation overrides
...
export default async function globalTeardown() {
  try {
    ...
    await pool.query(`DELETE FROM con_ed_allocation_overrides WHERE user_id IN ${testUsers}`);
