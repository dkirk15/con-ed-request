...
- Name: admin-user-management.spec.ts >> admin sees newly created user in Users directory
- Location: tests/admin-user-management.spec.ts:84:1
...
> 97  |     .toBe("employee");
  ...
  139 | 
  140 |   // After a page navigation the admin directory should show the new user.
  141 |   await page.goto("/users");
  142 |   await expect(
  143 |     page.getByRole("heading", { name: "Users", exact: true }),
  144 |   ).toBeVisible();
  145 |   await expect(page.getByText(newEmail)).toBeVisible();
  146 | 
  147 |   // Cleanup Clerk user if created.
  148 |   if (newClerkId) {
  149 |     await deleteClerkUser(newClerkId).catch(() => {});
  150 |   }
  151 | });
  152 | 
```