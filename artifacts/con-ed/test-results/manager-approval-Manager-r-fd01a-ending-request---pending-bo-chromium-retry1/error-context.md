...
- Name: manager-approval.spec.ts >> Manager review >> manager approves a pending request -> pending_bo
- Location: tests/manager-approval.spec.ts:15:3
...
Locator: locator('text=Action Required').locator('..').locator('text=Requests pending your approval')
Expected: visible
Timeout: 20000ms
...
Call log:
  ...
  - waiting for locator('text=Action Required').locator('..').locator('text=Requests pending your approval')
...
```ts
  ...
  3   | 
  4   | function requestIdFromUrl(url: string): number {
  5   |   const match = url.match(/\/requests\/(\d+)/);
  6   |   if (!match) throw new Error(`No request id in URL: ${url}`);
  7   |   return Number(match[1]);
  8   | }
  9   | 
  10  | /**
  11  |  * A manager reviews requests from their own clinic. Approving moves the request
  12  |  * to `pending_bo`; denying (with a reason) moves it to `manager_denied`.
  13  |  */
  14  | test.describe("Manager review", () => {
