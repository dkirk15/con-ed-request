...
- Receipt files are fetched through the authenticated API client and exposed
  only through temporary browser object URLs, preserving the existing request,
  clinic, and role access controls.
- The private storage route now returns the verified PDF/JPG/PNG media type and
